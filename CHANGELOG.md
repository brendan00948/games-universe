...
- **Auth gate:** **`/dashboard/`** redirects guests to **`/login.html?next=`** (internal paths only); **`init()`** runs only after sign-in. **`browserLocalPersistence`** on auth; login/signup honor **`next`** and pass it between pages.
